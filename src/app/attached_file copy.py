...
# CSV行を受け取る
if len(sys.argv) > 1:
    csv_rows = json.loads(sys.argv[1])
else:
    csv_rows = {}

# datetime に戻したい場合
if "観測日時" in csv_rows:
    csv_rows["観測日時"] = datetime.strptime(csv_rows["観測日時"], "%Y/%m/%d %H:%M")
# if "1年前日時" in csv_rows:
#     csv_rows["1年前日時"] = datetime.strptime(csv_rows["1年前日時"], "%Y/%m/%d %H:%M")


print("受け取りCSV:", csv_rows)
...
kinbou_data = []
for i in range(1, 4):
    KinbouName = csv_rows.get(f"近傍観測所{i}の名称")
    KinbouObsrvId = csv_rows.get(f"近傍観測所{i}のID")
    KinbouFlag = csv_rows.get(f"近傍観測所{i}フラグ")
...
KansokuName = csv_rows.get("観測所名")
ObsrvId = csv_rows.get("統一ID")
if ObsrvId and not ObsrvId.startswith("0"):
    ObsrvId = "0" + ObsrvId  # 先頭に0を付与
SuikeiName = csv_rows.get("水系名")
RiverName = csv_rows.get("河川名")
KanriKbn = csv_rows.get("管理区分")
Syubetu = csv_rows.get("項目種別")
...
DateNo = csv_rows.get("DateNo") # View 側で生成した DateNo をそのまま使用
clat = csv_rows.get("緯度") # View 側(MS_Kansokujoの緯度) をそのまま使用
clon = csv_rows.get("経度") # View 側(MS_Kansokujoの経度) をそのまま使用
ChihouCD = csv_rows.get("地方CD") # View 側(MS_Kansokujoの地方CD) をそのまま使用

# キャプチャ処理用の時刻を取得
Year, Month, Day, Hour, Minute = get_rireki_date(csv_rows)
...
for obsrv in obsrvId_list:
    if obsrv["Flag"]:  # FlagがTrueのものだけ処理
        ...
        # From
        #***************
        date_obj = csv_rows["観測日時"]   # ここで date_obj を定義
        #3時間前を設定
